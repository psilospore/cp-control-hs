...
for file in files:
    if '.pm' in file and not '_new.' in file:
        values=[]
        with open(file) as f:
            lines=f.readlines()
        ...
        print(file)
        print(len(allowable_acts))
        empty_sets=np.sum([len(act)>0 for [s_list, act] in allowable_acts])
        print(empty_sets,'\n\n')
        # [] cte_est0=0 & cte_est1=0 & cte_es2=0 & cte_est3=0 & cte_est4=0 & he_est0=0 & he_est1=0 & he_est2=0 & pc=2 -> (default'=2) & (pc'=3) ;
        ...
        with open(file[:-3]+'_new.pm', 'w') as new_file:
